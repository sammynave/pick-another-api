...
# version needs to match what is in `mix.exs`
ENV VERSION 0.0.1
# ENV MIX_HOST 80
# EXPOSE $MIX_HOST
# TODO: investigate why saving this to avar didn't work
# Could be in the nginx conf that ther is a \ before which is escaping it
# example: 127.0.0.1\$MIX_HOST or something ¯\_(ツ)_/¯
EXPOSE 80

RUN mkdir /app
WORKDIR /app
COPY ./rel/pick_another/releases/$VERSION/pick_another.tar.gz /app/pick_another.tar.gz
RUN tar -zxvf pick_another.tar.gz

WORKDIR /app/releases/$VERSION
ENTRYPOINT ["./pick_another.sh"]
CMD ["foreground"]
